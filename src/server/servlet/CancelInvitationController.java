...
private static Logger log = Logger.getLogger(CancelInvitationController.class);
...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if ((request.getAttribute("bad_time")) != null) {
				request.removeAttribute("bad_time");
			}
			if ((request.getAttribute("cancel_res")) != null) {
				request.removeAttribute("cancel_res");
			}
            ...
            if(fmt.format(datum_rez).equals(fmt.format(curr_date)) == true){
            	...
            	if ( datum_rez.getTime() - curr_date.getTime() < 30*60*1000) {
            		...
            		request.getSession().setAttribute("bad_time", bad_time);
            ...
            rezervacija.getPrihvaceniGosti().remove(kor);
